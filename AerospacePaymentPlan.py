...
if risktype == "*Select*":
  ...
  if submitlaunch:
    if depositPercent == 0:
      ...
      result = f"L2{endorsementMap[endorsementsCheck]} {(depositPercent/100):2.2%} Due on {depositdue.strftime('%m/%d/%Y')}. {(1-(depositPercent/100)):2.2%} Due {remainingDue:02d} Days Before Launch." + f"{f' END ${endorsementpremium:,.2f} Due {endorsementduedate}' if endorsementsCheck else ''}"
    st.divider()
    st.header(result)
